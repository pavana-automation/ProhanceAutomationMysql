...
public class ValidateActionAddAuditTrail extends BaseTest
{
	...
	public List<String> preparenotExeptedList()
	{
		...
		notExeptedValueList.add("LOGOUT");
	...
	@Test(retryAnalyzer = net.Commons.RetryAnalyzer.class, description = "Audit Trail actionAdd.")
	@Description("Custom Attribute actionAdd.")
	@Parameters("browser")
	public void actionAdd(String browser) throws InterruptedException, IOException
	{
		launchapp(browser);
		...
		List<String> specificColumnDataList = OrganizationsOutsidePage.getSpecificColumnDataInTable(1);
		workTemplate.unselectFrame();
		SideNavigationMenuPage.clickSideNavigationBtn();
		
		OrganizationsOutsidePage.sideMenuForOutsideOrganization("Support", "Audit Trail");
		workTemplate.iframeSwitch();
		
		List<String> dropDownValues = workTimeAuditTrailPage.getOrgDropDownValues();
		
		softAssert.assertEquals(dropDownValues, specificColumnDataList);
		...
		workTimeAuditTrailPage.selectActionChkBx("ADD");
