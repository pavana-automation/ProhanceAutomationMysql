...
public class OrganizationAllGroupLabel extends BaseTest
{
	...
	@Parameters("browser")
	public void organizationAllGroupLabel(String browser) throws InterruptedException
	{
		launchapp(browser);
		...
		driver.findElement(By.id("superGroupsLabel")).clear();
		driver.findElement(By.id("superGroupsLabel")).sendKeys("Super Groups-test");
		orgPage.clickOnSaveBtn();
		
		Thread.sleep(5000);
		
		String expected = driver.findElement(By.xpath("//input[@id='superGroupsLabel']")).getText();
		
		System.out.println("the expected is :" +expected);
		
		driver.findElement(By.id("superGroupsLabel")).clear();
		driver.findElement(By.id("superGroupsLabel")).sendKeys("Super Groups");
		orgPage.clickOnSaveBtn();
